...
const ResumeBuilder = () => {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      navigator.share({ url: resumeUrl, text: "My Rresume" });
    } else {
  ...
  const saveResume = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || error.message);
      throw error; // rethrow so toast.promise's error path works
    }
  };
            ...
            <div className="relative w-full">
              <div className="absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2">
                {resumeData.public && (
                  <button
                    onClick={handleShare}
                    className="flex items-center p-2 px-4 gap-2 text-xs bg-linear-to-br from-blue-100 to-blue-200 transition-colors"
                  >
